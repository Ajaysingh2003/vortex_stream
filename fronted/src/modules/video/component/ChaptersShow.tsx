...
import { VideoAsset, VideoChapter, WorkspaceType } from "@/modules/types";
...
function ChaptersShow({ isPremium }: { isPremium: boolean }) {
  ...
  const chapterData = chaptersData as VideoChapter[];

  console.log(chaptersData, "chapter are here");

  const mutateChapter = useMutation(
    trpc.video.VideoChapter.mutationOptions({
      onSuccess: () => {
  ...
  const convertChapters = chapterData.map((e) => ({
    startTime: e.time,
    name: e.label,
    id: e.id,
  }));
  ...
  const [chapters, setChapters] = useState<Chapter[]>(() =>
    convertChapters.length == 0
      ? [{ id: makeId(), startTime: "00:00", name: "" }]
      : convertChapters,
  );

  const mutateDeleteChapter = useMutation(
    trpc.video.deleteVideoChapter.mutationOptions({
        onSuccess:()=>{
            toast.success("Video Chapter Removed Successfully")
        },
  ...
  const removeChapter = async (id: string) => {
    ...
    const fromDatabase = chapterData.find((e) => e.id == id);

    if (!fromDatabase) return;

    await mutateDeleteChapter.mutateAsync({
      id: id,
      workspaceId: workspaceData.id,
    });
  };
