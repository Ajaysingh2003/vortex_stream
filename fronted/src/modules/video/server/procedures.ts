...
export const videoRouter = createTRPCRouter({
  ...
  getVideoOverview: getUserProcedure
    .input(
      z.object({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const response = await axios.get(
          `${process.env.BASE_API}/v1/workspace/${input.workspaceId}/video/overview`,
          {
            headers: {
        ...
        if (axios.isAxiosError(error)) {
          ...
          throw new TRPCError({
            code,
            message: error.response?.data?.message || "Unable to load video overview",
          });
        }

        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Unable to load video overview",
        });
      }
    }),
  ...
  VideoCta: baseProcedure
        ...
        items: z
          .array(
            z.object({
              ...
              open_in: z
                .string()
                .min(1, { message: "Open in is required" })
                .trim(),
              font_color: z
                .string()
                .min(1, { message: "Font color is required" })
                .trim(),
              background_color: z
                .string()
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const id = input.video_id;
        const res = await axios.post(
          `${process.env.BASE_API}/v1/workspace/${input.workspaceID}/video/${id}/cta`,
          { ...input },
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
  ...
  getVideoCtas: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        const res = await axios.get(
          `${process.env.BASE_API}/v1/video/${input.videoId}/cta`,
        );
  ...
  deleteVideoChapter: baseProcedure
    .input(
      z.object({
        workspaceId: z.string(),
        id: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const res = await axios.delete(
          `${process.env.BASE_API}/v1/workspace/${input.workspaceId}/video/chapter/${input.id}`,
          {
            headers: {
              Authorization: `Bearer ${access_token}`,
              // "Content-Type": "application/json",
  ...
  deleteVideoCta: getUserProcedure
    .input(
      z.object({
        workspaceId: z.string(),
        id: z.string(),
