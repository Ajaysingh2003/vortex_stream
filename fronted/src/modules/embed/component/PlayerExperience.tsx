...
export function ExperienceOverlay({ experience, state, videoId, cdnBaseUrl }: {
  ...
  return <div className="pointer-events-none absolute inset-0 z-40">
    {experience.ctas.filter(cta => state.currentTime >= timeInSeconds(cta.start_time) && state.currentTime < timeInSeconds(cta.end_time)).map(cta => {
      ...
      return href ? <a key={cta.id} href={href} target={cta.open_in === "_self" || cta.open_in === "same_tab" ? "_self" : "_blank"} rel="noopener noreferrer" className={`pointer-events-auto absolute max-w-[calc(100%-2rem)] break-words shadow-lg ${primaryButton} ${positions[cta.position] || positions.top_right}`} style={{ backgroundColor: /^#[\da-f]{6}$/i.test(cta.background_color) ? cta.background_color : undefined, color: /^#[\da-f]{6}$/i.test(cta.font_color) ? cta.font_color : undefined }}>
...
function ViewerEndScreen({ endScreen, onReplay, cdnBaseUrl }: { endScreen: VideoEndScreenType | null; onReplay: () => void; cdnBaseUrl: string }) {
  ...
  return <div className="flex min-h-full flex-col items-center justify-center gap-5 p-5 text-center text-white sm:p-8">
    <div className="w-full max-w-2xl space-y-4">
      ...
      })}</div><button className={primaryButton} onClick={async () => { try { await navigator.clipboard.writeText(window.location.href); setCopied(true); setCopyError(false); } catch { setCopyError(true); } }}>{copied ? <Check className="size-4" /> : <Copy className="size-4" />}{copied ? "Link copied" : "Copy video link"}</button>{copyError && <p role="alert" className="text-xs text-white/70">Copy the video URL from your browser to share it.</p>}</> : <h2 className="text-xl font-semibold">Thanks for watching</h2>}
    </div>
  ...
  return <div ref={root} className="absolute inset-x-3 bottom-[4.5rem] z-[350] flex justify-end gap-2 sm:inset-x-4 sm:bottom-20" onKeyDown={event => { event.stopPropagation(); if (event.key === "Escape") { setMenu(null); root.current?.querySelector<HTMLButtonElement>(`button[data-trigger="${menu}"]`)?.focus(); } }}>
    {menu && <section aria-label={menu === "chapters" ? "Video chapters" : "Subtitle languages"} className="absolute bottom-11 right-0 w-72 max-w-full overflow-hidden rounded-xl border border-white/15 bg-neutral-950/95 text-white shadow-2xl backdrop-blur-xl">
      ...
      <div className="max-h-[min(16rem,max(3rem,calc(100cqh-10rem)))] overflow-y-auto p-1.5">
        {menu === "chapters" ? chapters.map((chapter, index) => <button key={chapter.id} disabled={disableSeek} aria-current={active === chapter.id ? "true" : undefined} onClick={() => { const video = videoRef.current; if (video) video.currentTime = Math.min(chapter.seconds, Number.isFinite(video.duration) ? video.duration : chapter.seconds); setMenu(null); }} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-xs transition disabled:cursor-not-allowed disabled:opacity-50 ${active === chapter.id ? "bg-primary/15 text-primary" : "text-white/80 hover:bg-white/10"}`}><span className="text-[10px] tabular-nums text-white/40">{String(index + 1).padStart(2, "0")}</span><span className="min-w-0 flex-1 break-words">{chapter.label}</span><span className="shrink-0 tabular-nums text-white/50">{chapter.time}</span></button>) : [{ code: "off", label: "Off" }, ...experience.subtitles].map(track => <button key={track.code} aria-pressed={selectedLanguage === track.code} onClick={() => setLanguage(track.code)} className="flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-xs hover:bg-white/10"><span>{track.label}</span>{selectedLanguage === track.code && <Check className="size-4 text-primary" />}</button>)}
      </div>
